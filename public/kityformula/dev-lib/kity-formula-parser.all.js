...
(function () {
    ...
    _p[0] = {
        value: function () {
            ...
            function generateExpression(
                ...
                if (tree.name.indexOf('text') === -1) {
                    ...
                    for (var i = 0, len = operand.length; i < len; i++) {
                        ...
                        if (!currentOperand) {
                            ...
                            operand[i] = generateExpression(
                                originTree.operand[i],
                                currentOperand,
                                objTree.operand[objTree.operand.length - 1],
                                mapping,
                                selectInfo
                            );
                        }
                ...
                if (!constructor) {
                    throw new Error(
                        'operator type error: not found ' + tree.operator
                    );
                }
            ...
            function processFunction(units) {
                ...
                while ((currentUnit = units.pop()) !== undefined) {
                    if (
                        ...
                        (currentUnit.sign === false ||
                            currentUnit.name === 'function')
                    ) {
                        // 预先处理不可作为独立符号的函数
                        var tt = currentUnit.handler(
                            currentUnit,
                            [],
                            processed.reverse()
                        );
            ...
            function generateTree(units) {
                ...
                while ((currentUnit = units.shift())) {
                    if (
                        ...
                        // 后操作数
                        tree.push(
                            currentUnit.handler(currentUnit, tree, units)
                        );
                    } else {
    ...
    _p[4] = {
        value: function (require) {
            var OPERATOR_LIST = _p.r(7),
                FUNCTION_LIST = _p.r(6),
                FUNCTION_HANDLER = _p.r(15),
                Utils = {
    ...
    _p[16] = {
        value: function (require) {
            ...
            return function (info, processedStack, unprocessedStack) {
                ...
                if (
                    params.expr &&
                    params.expr.type === FN_TYPE &&
                    params.expr.handler
                ) {
                    params.expr = params.expr.handler(
                        params.expr,
                        processedStack,
                        unprocessedStack
                    );
    ...
    _p[17] = {
        value: function () {
            ...
            function extract(stack) {
                ...
                if (
                    forward.name === 'subscript' ||
                    forward.name === 'superscript'
                ) {
                    return {
                        type: forward.name,
                        value: stack.shift()
    ...
    _p[25] = {
        value: function (require) {
            var Parser = _p.r(43).Parser,
                LatexUtils = _p.r(1),
                PRE_HANDLER = _p.r(8),
                serialization = _p.r(42),
                OP_DEFINE = _p.r(7),
                REVERSE_DEFINE = _p.r(9),
                SPECIAL_LIST = _p.r(10),
                Utils = _p.r(4);
            // data
            var leftChar = '￸',
                rightChar = '￼',
    ...
    _p[42] = {
        value: function (require) {
            var reverseHandlerTable = _p.r(9),
                SPECIAL_LIST = _p.r(10),
            ...
            function reverseParse(tree, options) {
                ...
                if (
                    tree.name === 'combination' &&
                    tree.operand.length === 1 &&
                    tree.operand[0].name === 'combination'
                ) {
                    tree = tree.operand[0];
                }
                ...
                return reverseHandlerTable[reverseHandlerName].call(
                    tree,
                    operands,
                    options
                );
