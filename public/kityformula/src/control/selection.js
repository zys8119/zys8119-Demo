...
define(function (require, exports, module) {
    ...
    return kity.createClass('SelectionComponent', {
        ...
        initEvent: function () {
            var eventServiceObject = this.kfEditor.request(
            ...
            eventServiceObject.on('mousedown', function (e) {
                ...
                // 存在根占位符， 禁止拖动
                if (
                    _self.kfEditor.requestService('syntax.has.root.placeholder')
                ) {
                    return false;
                }
            ...
            eventServiceObject.on('mousemove', function (e) {
                ...
                if (!_self.isDrag) {
                    if (_self.isMousedown) {
                        ...
                        if (
                            MAX_DISTANCE <
                            _self.getDistance(e.clientX, e.clientY)
                        ) {
        ...
        updateSelection: function (target, x, y) {
            ...
            if (currentGroupInfo.groupInfo.id === startGroupInfo.groupInfo.id) {
                ...
                if (
                    ...
                    // 获取公共容器
                    communityGroupInfo = this.getCommunityGroup(
                        startGroupInfo.groupInfo,
                        currentGroupInfo.groupInfo
                    );

                    // 修正偏移相同时的情况， 比如在分数中选中时
                    if (
                        communityGroupInfo.startOffset ===
                        communityGroupInfo.endOffset
                    ) {
                        ...
                        // 当前光标移动所在的组元素节点
                        currentGroupNode =
                            communityGroupInfo.group.content[
                                communityGroupInfo.endOffset
                            ];

                        inRightArea = this.kfEditor.requestService(
                            'position.get.area',
                            currentGroupNode,
                            x
                        );
        ...
        updateSelectionByTarget: function (target) {
            var parentGroupInfo = this.kfEditor.requestService(
            ...
            if (
                ...
                if (
                    ...
                    // 获取包含父组的容器
                    containerInfo = this.kfEditor.requestService(
                        'position.get.group.info',
                        parentGroupInfo.groupObj
                    );
        ...
        selectAll: function () {
            var rootGroupInfo = this.kfEditor.requestService(
        ...
        getGroupInof: function (offset, target) {
            var groupInfo = this.kfEditor.requestService(
                'position.get.group',
                target
            );
            ...
            var index = this.kfEditor.requestService(
        ...
        getCommunityGroup: function (startGroupInfo, endGroupInfo) {
            ...
            while (
                (bigBoundingGroup = this.kfEditor.requestService(
                    'position.get.group.info',
                    targetGroup
                ))
            ) {
                ...
                if (
                    kfUtils.contains(
                        bigBoundingGroup.group.groupObj,
                        endGroupInfo.groupObj
                    )
                ) {
                    break;
                }
