...
define(function (require, exports, module) {
    ...
    return kity.createClass('LocationComponent', {
        ...
        updateCursorInfo: function (evt) {
            ...
            wrapNode = this.kfEditor.requestService(
                'position.get.wrap',
                evt.target
            );
        ...
        updateCursor: function () {
            ...
            var groupInfo = this.kfEditor.requestService(
                ...
                canvasZoom = this.kfEditor.requestService(
                    'render.get.canvas.zoom'
                ),
                formulaZoom = this.paper.getZoom();

            this.cursorShape.setHeight(
                focusChildRect.height / canvasZoom / formulaZoom
            );

            // 计算光标偏移位置
            cursorOffset = isBefore
                ? focusChildRect.left - 2
                : focusChildRect.left + focusChildRect.width - 2;
            cursorOffset -= paperContainerRect.left;

            // 定位光标
            cursorTransform.m.e =
                Math.floor(cursorOffset / canvasZoom / formulaZoom) + 0.5;
            cursorTransform.m.f =
                (focusChildRect.top - paperContainerRect.top) /
                canvasZoom /
                formulaZoom;
