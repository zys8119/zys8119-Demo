...
define(function (require) {
    ...
    return kity.createClass('Scrollbar', {
        ...
        initSize: function () {
            ...
            this.values.trackWidth =
                this.values.viewWidth - leftBtnWidth - rightBtnWidth;
        ...
        update: function (contentWidth) {
            ...
            thumbWidth = Math.max(
                Math.ceil((trackWidth * trackWidth) / contentWidth),
                SCROLLBAR_DEF.thumbMinSize
            );
        ...
        updateOffset: function (offset) {
            ...
            this.leftOverflow =
                values.left * (values.contentWidth - values.viewWidth);
            this.rightOverflow =
                values.contentWidth - values.viewWidth - this.leftOverflow;

            this.updateHandler(values.left, values.offset, values);
        },

        relocation: function () {
            var cursorLocation = this.kfEditor.requestService(
                ...
                // 视图左溢出长度
                viewLeftOverflow =
                    this.values.left * (contentWidth - viewWidth),
                diff = 0;
            ...
            if (cursorLocation.x < viewLeftOverflow) {
                ...
                if (this.isExpand) {
                    ...
                    // 减少左溢出
                    setThumbByLeftOverflow(
                        this,
                        contentWidth - viewWidth - this.rightOverflow
                    );
                }
            }
        }
    ...
    function trackClickHandler(track, comp, evt) {
        var trackRect = getRect(track),
            ...
            unitOffset =
                (values.viewWidth / (values.contentWidth - values.viewWidth)) *
                values.trackWidth,
            // 点击位置在轨道中的偏移
    ...
    function setThumbLocation(comp, locationX) {
        ...
        var values = comp.values,
            ...
            offset = Math.floor(
                (locationX / trackPieceWidth) * values.trackWidth
            );
    ...
    function setThumbOffsetByViewOffset(comp, viewOffset) {
        ...
        // 轨道偏移比例
        offsetProportion =
            viewOffset / (values.contentWidth - values.viewWidth);
    ...
    function setThumbByLeftOverflow(comp, leftViewOverflow) {
        var values = comp.values,
            overflowProportion =
                leftViewOverflow / (values.contentWidth - values.viewWidth);
