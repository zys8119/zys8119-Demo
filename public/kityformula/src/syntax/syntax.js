    ...
    var kity = require('kity'),
        ...
        SyntaxComponenet = kity.createClass('SyntaxComponenet', {
            ...
            initComponents: function () {
                ...
                this.components.delete = new DeleteComponent(
                    this,
                    this.kfEditor
                );
            },
            ...
            updateObjTree: function (objTree) {
                ...
                if (selectInfo && selectInfo.groupId) {
                    this.updateCursor(
                        selectInfo.groupId,
                        selectInfo.startOffset,
                        selectInfo.endOffset
                    );
                }
            ...
            isGroupNode: function (groupId) {
                var type =
                    this.objTree.mapping[groupId].strGroup.attr['data-type'];
            ...
            isBrackets: function (groupId) {
                return !!this.objTree.mapping[
                    groupId
                ].objGroup.node.getAttribute('data-brackets');
            },
            ...
            isSelectPlaceholder: function () {
                ...
                if (
                    !this.isPlaceholder(
                        groupInfo.content[cursorInfo.startOffset].id
                    )
                ) {
                    return false;
                }
            ...
            updateSelection: function (group) {
                ...
                if (curStrGroup.name === 'combination') {
                    ...
                    // 函数处理， 找到函数所处的最大范围
                    while (
                        parentGroupObj.strGroup.name !== 'combination' ||
                        parentGroup.content === 1
                    ) {
                        group = parentGroup;
                        groupObj = parentGroupObj;

                        parentGroup = this.kfEditor.requestService(
                            'position.get.parent.group',
                            groupObj.objGroup.node
                        );
                    ...
                    // 在当前函数所在的位置作标记
                    parentGroupObj.strGroup.operand.splice(
                        parentIndex + 1,
                        0,
                        CURSOR_CHAR
                    );
                ...
                resultStr = this.kfEditor.requestService(
                ...
                parentGroupObj.strGroup.operand.splice(
                    this.record.cursor.startOffset,
                    1
                );
                parentGroupObj.strGroup.operand.splice(
                    this.record.cursor.endOffset,
                    1
                );
            ...
            insertSubtree: function (subtree) {
                ...
                if (this.isPlaceholder(cursorInfo.groupId)) {
                    ...
                    currentTree =
                        this.objTree.mapping[cursorInfo.groupId].strGroup;
            ...
            replaceTree: function (subtree) {
                var cursorInfo = this.record.cursor,
                    groupNode =
                        this.objTree.mapping[cursorInfo.groupId].objGroup.node,
                    parentInfo = this.kfEditor.requestService(
                        'position.get.parent.info',
                        groupNode
                    ),
                    currentTree =
                        this.objTree.mapping[parentInfo.group.id].strGroup;
