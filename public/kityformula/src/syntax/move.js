...
define(function (require, exports, module) {
    ...
    function updateCursorGoLeft(cursorInfo) {
        ...
        if (syntaxComponent.isSelectPlaceholder()) {
            return locateOuterIndex(
                this,
                containerInfo.content[cursorInfo.startOffset],
                DIRECTION.LEFT
            );
        }
        ...
        if (cursorInfo.startOffset === cursorInfo.endOffset) {
            if (cursorInfo.startOffset > 0) {
                prevGroupNode =
                    containerInfo.content[cursorInfo.startOffset - 1];

                if (isGroupNode(prevGroupNode)) {
                    cursorInfo = locateIndex(
                        this,
                        prevGroupNode,
                        DIRECTION.LEFT
                    );
                ...
                cursorInfo = locateOuterIndex(
                    this,
                    containerInfo.groupObj,
                    DIRECTION.LEFT
                );
            }
        } else {
            cursorInfo.startOffset = Math.min(
        ...
        if (isGroupNode(groupNode)) {
            ...
            if (isContainerNode(groupNode)) {
                ...
                if (isPlaceholderNode(groupElement)) {
                    ...
                    isContainerNode(groupElement) &&
                    groupInfo.content.length === 1
                ) {
                ...
                while (
                    ...
                    !isPlaceholderNode(groupElement)
                ) {
                    groupInfo = syntaxComponent.getGroupContent(
                        groupElement.id
                    );
                    groupElement =
                        groupInfo.content[groupInfo.content.length - 1];
                }
    ...
    function locateOuterLeftIndex(moveComponent, groupNode) {
        ...
        outerGroupInfo = kfEditor.requestService(
        ...
        while (outerGroupInfo.index === 0) {
            ...
            if (
                isContainerNode(outerGroupInfo.group.groupObj) &&
                outerGroupInfo.group.content.length > 1
            ) {
                return {
                    groupId: outerGroupInfo.group.id,
                    startOffset: 0,
    ...
    function isGroupNode(node) {
        ...
        return (
            dataType === 'kf-editor-group' ||
            dataType === 'kf-editor-virtual-group'
        );
    }
