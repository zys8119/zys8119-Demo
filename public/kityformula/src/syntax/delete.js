...
define(function (require, exports, module) {
    ...
    return kity.createClass('DeleteComponent', {
            ...
            if (cursorInfo.startOffset === cursorInfo.endOffset) {
                ...
                if (cursorInfo.startOffset === 0) {
                    ...
                    if (currentTree.operand.length > 1) {
                        cursorInfo = this.deletePrevGroup(
                            currentTree,
                            cursorInfo
                        );
                ...
                if (this.parentComponent.isSelectPlaceholder()) {
                    ...
                    if (this.parentComponent.isRootTree(currentTree)) {
                        ...
                        cursorInfo = this.selectParentContainer(
                            cursorInfo.groupId
                        );
                        this.parentComponent.updateCursor(cursorInfo);
        ...
        deleteSelection: function (tree, cursorInfo) {
            ...
            if (
                cursorInfo.startOffset === 0 &&
                cursorInfo.endOffset === tree.operand.length
            ) {
        ...
        selectParentContainer: function (groupId) {
            var currentGroupNode =
                    this.parentComponent.getGroupObject(groupId).node,
                parentContainerInfo = this.kfEditor.requestService(
                    'position.get.group',
                    currentGroupNode
                ),
                // 当前组在父容器中的索引
                index = this.kfEditor.requestService(
                    'position.get.index',
                    parentContainerInfo.groupObj,
                    currentGroupNode
                );
