...
function toHex8(value) {
  ...
  return chunk(color.slice(2) + color.slice(0,2),2).map(e => parseInt(e.join(''),16)).reduce((a, b,k) => (a[['r','g','b','a'][k]]= b , a),{})
}
onMounted(async ()=>{
    ...
    switch (e.penType){
      case "UNDERWAVELINE":
        ...
        await Promise.all([getRect(e.data.data)].map(async ee=>{
          ...
          ctx.strokeStyle = `rgba(${color.red*255}, ${color.green*255}, ${color.blue*255}, ${alpha})`; // Set the wave color
