<template>
    <div class="ScrollUpTable abs-content p-x-10px flex flex-col gap-5px">
        <div class="flex gap-5px bg-#141754 flex-center-start p-y-10px b-rd-10px">
            <div :class="item.className" class="flex-center" v-for="(item, index) in columns" :key="index">{{
                item.title }}</div>
        </div>
            ...
            <ScrollUp :list="data">
                <template #default="{ item: row, index }">
                    <div class="flex gap-5px  flex-center-start p-y-10px b-rd-10px">
                        <div :class="col.className" class="flex-center" v-for="(col, index) in columns" :key="index">
