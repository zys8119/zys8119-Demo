...
const getRoute = (
  ...
  const result = {
    ...
    ...(Object.prototype.toString.call(routeProp) === "[object Object]"
      ? routeProp
      : {}),
  } as RouteRecordRaw;
    ...
    if (!ROUTES_FILTER_REG.test(key)) {
      ...
      for (let j = 0; j < chain.length; j++) {
        ...
        if (lastNode == currentNode) {
          ...
          const path = `./VIEWSDIR/${chain.slice(0, j + 1).join("/")}${
            directory ? "" : `.${suffix}`
          }`;
          let layoutComponent: any = RouterView;
          if (directory) {
            const layoutPath = input.find((e) =>
              new RegExp(`^${path}\/layout\\.(vue|jsx|tsx)`, "img").test(e)
            );
            if (layoutPath) {
              layoutComponent = files[layoutPath];
            }
          }
          const pageJsonPath = directory
            ? `${path}/page.json`
            : path.replace(
          ...
          const meta = directory
            ? directoryMeta[chain[j]] || {}
            : Object.assign(
                metaMaps[path] || {},
                (pages[pageJsonPath] || ({} as any))[`${chain[j]}.${suffix}`] ||
                  {}
              );
          const route_path =
            typeof meta?.path === "string" ? meta.path : chain[j].toLowerCase();
